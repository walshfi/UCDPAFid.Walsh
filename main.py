...
# Group the Bestsellers by year and find the min, max, and average for prices and reviews
...
AMZN_close = np.array(AMZN['close'])
print(AMZN_close)
close_mean = np.mean(AMZN_close)
print(close_mean)
...
ax.plot(FB.index, FB['close'], color='b')
...
fig, ax = plt.subplots()
ax.set_title('Facebook closing stock price and volume')
...
ax2.plot(FB.index, FB["volume"], color='r')
...
# Set the second Y label to Volume
...
# Add the time-series for "close" to the plot
ax.plot(AMZN.index, AMZN['close'], color='m')
...
fig, ax = plt.subplots()
ax.set_title('Amazon closing stock price and volume')
# Plot the close variable in blue
ax.plot(AMZN.index, AMZN["close"], color='m')
...
# Plot the volume in red
ax2.plot(AMZN.index, AMZN["volume"], color='g')
